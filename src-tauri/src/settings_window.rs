...
use objc2::{msg_send, sel, ClassType, rc::Retained};
use objc2::runtime::{ClassBuilder, Sel, AnyClass};
use objc2_foundation::{MainThreadMarker, NSObject, NSString, NSPoint, NSRect, NSSize, NSEdgeInsets, NSArray};
use objc2_app_kit::{
    NSButton, NSLayoutConstraint, NSPanel, NSStackView, NSTextField, NSPopUpButton,
    NSWindowStyleMask, NSBackingStoreType, NSUserInterfaceLayoutOrientation, NSGridView, 
    NSColor, NSFont, NSBox, NSBezelStyle, NSSwitch, NSView,
};
...
fn create_settings_delegate(_mtm: MainThreadMarker) -> Retained<NSObject> {
    ...
    REGISTER.call_once(|| {
        ...
        let name = CStr::from_bytes_with_nul(b"EyeBreakSettingsDelegate\0").unwrap();
        let mut builder = ClassBuilder::new(name, superclass).expect("failed to create class builder");
...
extern "C" fn save_action(_this: &NSObject, _cmd: Sel, _sender: Option<&NSObject>) {
    ...
    let guard = SETTINGS_CONTROLS.get_or_init(|| Mutex::new(None)).lock().unwrap();
    if let Some(wrapper) = &*guard {
        ...
        let theme = controls.theme_popup.titleOfSelectedItem()
            .map(|s| s.to_string()).unwrap_or("dark".to_string());
        let sound = controls.sound_popup.titleOfSelectedItem()
            .map(|s| s.to_string()).unwrap_or("off".to_string());
        let warn_val = controls.warning_popup.titleOfSelectedItem()
            .map(|s| s.to_string()).unwrap_or("Off".to_string());
        let pre_warn = if warn_val == "Off" { 0 } else {
            warn_val.trim_end_matches('s').parse().unwrap_or(60)
        };

        log::info!("Saving: Work={}, Break={}, Strict={}, Launch={}, Meet={}, Theme={}, Sound={}, Warn={}", 
                   work_mins, break_secs, strict, launch, meet, theme, sound, pre_warn);
...
fn close_settings_window() {
    let guard = SETTINGS_WINDOW.get_or_init(|| Mutex::new(None)).lock().unwrap();
    if let Some(wrapper) = &*guard {
        wrapper.0.close();
    }
}
...
pub fn show_settings(app: &AppHandle) {
    ...
    let mut guard = SETTINGS_WINDOW.get_or_init(|| Mutex::new(None)).lock().unwrap();
    if let Some(wrapper) = &*guard {
    ...
    main_stack.setEdgeInsets(NSEdgeInsets { top: 20.0, left: 20.0, bottom: 20.0, right: 20.0 });
    main_stack.setTranslatesAutoresizingMaskIntoConstraints(false);
    
    content_view.addSubview(&*main_stack);
    
    // Constraints
    let c1 = main_stack.topAnchor().constraintEqualToAnchor(&*content_view.topAnchor());
    let c2 = main_stack.leadingAnchor().constraintEqualToAnchor(&*content_view.leadingAnchor());
    let c3 = main_stack.trailingAnchor().constraintEqualToAnchor(&*content_view.trailingAnchor());
    ...
    let grid_timer: Option<Retained<NSGridView>> = unsafe { msg_send![mtm.alloc::<NSGridView>(), initWithFrame: NSRect::ZERO] };
    ...
    // Work Interval
    let (lbl_work, input_work) = create_number_row("Work interval (minutes)", work_mins as i32, mtm);
    let views_work: [&NSView; 2] = [&*lbl_work, &*input_work];
    grid_timer.addRowWithViews(&NSArray::from_slice(&views_work));
    
    // Break Duration
    let (lbl_break, input_break) = create_number_row("Break duration (seconds)", break_secs as i32, mtm);
    ...
    let grid_behavior: Option<Retained<NSGridView>> = unsafe { msg_send![mtm.alloc::<NSGridView>(), initWithFrame: NSRect::ZERO] };
    ...
    let grid_appearance: Option<Retained<NSGridView>> = unsafe { msg_send![mtm.alloc::<NSGridView>(), initWithFrame: NSRect::ZERO] };
    ...
    let (lbl_theme, popup_theme) = create_dropdown_row("Overlay Theme", &["dark", "light", "nature"], &theme, mtm);
    let views_theme: [&NSView; 2] = [&*lbl_theme, &*popup_theme];
    grid_appearance.addRowWithViews(&NSArray::from_slice(&views_theme));
    
    let (lbl_sound, popup_sound) = create_dropdown_row("Timer Sound", &["off", "chime", "whitenoise"], &sound, mtm);
    let views_sound: [&NSView; 2] = [&*lbl_sound, &*popup_sound];
    grid_appearance.addRowWithViews(&NSArray::from_slice(&views_sound));
    
    let warn_str = if pre_warn == 0 { "Off".to_string() } else { format!("{}s", pre_warn) };
    let (lbl_warn, popup_warn) = create_dropdown_row("Pre-break Warning", &["Off", "30s", "60s", "90s", "120s"], &warn_str, mtm);
    ...
    *SETTINGS_CONTROLS.get_or_init(|| Mutex::new(None)).lock().unwrap() = Some(SettingsControlsWrapper(controls));

    // Keep delegate alive
    *SETTINGS_DELEGATE.get_or_init(|| Mutex::new(None)).lock().unwrap() = Some(DelegateWrapper(delegate));
...
fn create_number_row(label_text: &str, default_val: i32, mtm: MainThreadMarker) -> (Retained<NSTextField>, Retained<NSTextField>) {
    let label = NSTextField::new(mtm);
    ...
    input.widthAnchor().constraintEqualToConstant(60.0).setActive(true);
    (label, input)
}

fn create_switch_row(label_text: &str, checked: bool, mtm: MainThreadMarker) -> (Retained<NSTextField>, Retained<NSSwitch>) {
    let label = NSTextField::new(mtm);
    label.setStringValue(&NSString::from_str(label_text));
    label.setBezeled(false);
    label.setDrawsBackground(false);
...
fn create_dropdown_row(label_text: &str, options: &[&str], selected: &str, mtm: MainThreadMarker) 
    -> (Retained<NSTextField>, Retained<NSPopUpButton>) 
{
    ...
    let popup: Option<Retained<NSPopUpButton>> = unsafe { 
        msg_send![popup, initWithFrame: NSRect::ZERO, pullsDown: false] 
    };
